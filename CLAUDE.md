...
- French is the default and the reference on every URL; English only when the visitor picks it with the switch (never detected from the browser). Legal pages in English show a note that the French version prevails.
...
- Theming: use the `ls-*` tokens (CSS variables in `src/index.css`, mapped in `tailwind.config.js`). Light theme only: no dark mode, no `dark:` classes. No raw grays or hex colors in components.
