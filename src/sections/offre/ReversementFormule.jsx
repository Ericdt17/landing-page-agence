...
const ReversementFormule = ({ id, kicker, withExample = false }) => {
  const { reversement } = useCopy("offre");
  const [collected, fees, debts] = reversement.formulaTerms;
    ...
    <section aria-labelledby={id} className='px-[18px] pb-16 md:px-16 md:pb-[84px]'>
      <div className='grid grid-cols-1 gap-8 border-t border-ls-rule pt-11 lg:grid-cols-[380px_minmax(0,1fr)] lg:gap-[72px]'>
        <div className='flex flex-col gap-4'>
          <span className='ls-kicker text-ls-accent'>{kicker ?? reversement.kicker}</span>
          <h2 id={id} className='ls-h ls-d2'>
            {reversement.title}
          </h2>
          <p className='ls-body text-ls-muted'>{reversement.body}</p>
        </div>

        <div className='flex flex-col justify-center gap-5'>
          <p className='flex flex-wrap items-center gap-3 rounded-[22px] border border-ls-rule bg-ls-surface p-6'>
            <span className='sr-only'>{reversement.formulaSr}</span>
            <span aria-hidden='true' className='ls-h text-lg'>
              {reversement.formulaResult}
            </span>
            <Operator>=</Operator>
            <Term strong>{collected}</Term>
            <Operator>−</Operator>
            <Term>{fees}</Term>
            <Operator>−</Operator>
            <Term>{debts}</Term>
          ...
          {withExample && (
            <div>
              <p className='ls-kicker pb-3 text-ls-faint'>{reversement.exampleLabel}</p>
              <dl className='grid grid-cols-2 gap-px border-y border-ls-rule bg-ls-rule md:grid-cols-4'>
                {reversement.example.map((cell) => (
                  <div
                    key={cell.label}
          ...
          <p className='ls-cap text-ls-muted'>{reversement.note}</p>
        </div>
      </div>
    </section>
  );
